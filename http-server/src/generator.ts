...
class AetheriaStaticGenerator {
  ...
  constructor(outputPath: string = '../generated') {
...
`;
  ...
  private async generateForDataType(dataType: string): Promise<void> {
  ...
  private async generateDataTypeIndex(dataType: string, data: any, outputDir: string): Promise<void> {
...
`;
  ...
  private async generateEntityFiles(dataType: string, data: any, outputDir: string): Promise<void> {
    const entities = this.getAllEntities(data);
  ...
  private async generateEntityFile(dataType: string, entity: any, outputDir: string): Promise<void> {
  ...
  private generateBasicEntityMarkdown(entityName: string, entity: any, dataType: string): string {
  ...
  private objectToMarkdown(obj: any, indent: number = 0): string {
  ...
  private getAllEntities(data: any, path: string = ''): Array<{key: string, name: string, path: string, data: any}> {
    const entities: Array<{key: string, name: string, path: string, data: any}> = [];
        ...
        for (const [key, value] of Object.entries(obj)) {
          ...
          if (typeof value === 'object' && value !== null) {
            ...
            if ((value as any).children) {
              traverse((value as any).children, newPath);
            }
          }
        }
      }
  ...
  private renderHierarchyText(data: any, indent: number = 0): string {
    const spaces = '  '.repeat(indent);
    let result = '';
    ...
    for (const [key, value] of Object.entries(data)) {
      ...
      if (typeof value === 'object' && value !== null && (value as any).children) {
        result += this.renderHierarchyText((value as any).children, indent + 1);
      }
    }
...
async function ensureGenericTemplate(): Promise<void> {
