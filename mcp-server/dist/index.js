...
class AetheriaMCPServer {
    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            switch (name) {
                case 'search_lore':
                    return await this.searchLore(args.query, args.category, args.includeContext, args.maxResults);
                case 'get_hierarchy':
                    return await this.getHierarchy(args.type, args.entity, args.includeChildren, args.includeParents);
                case 'generate_content':
                    return await this.generateContent(args.type, args.name, args.parameters);
                case 'extract_data':
                    return await this.extractData(args.source, args.dataType, args.format);
                default:
    ...
    async searchLore(query, category = 'all', includeContext = true, maxResults = 10) {
        const results = [];
        const markdownFiles = await this.findMarkdownFiles();
        for (const file of markdownFiles) {
    ...
    async getHierarchy(type, entity, includeChildren = true, includeParents = true) {
        try {
            let data;
            ...
            return {
                content: [{
                        type: 'text',
                        text: `No hierarchy data found for ${type}. Error: ${error.message}`,
                    }],
            };
        }
    }
    async extractHierarchyFromMarkdown(type) {
        const markdownFiles = await this.findMarkdownFiles();
        const relevantFile = markdownFiles.find(f => path.basename(f, '.md').toLowerCase() === type.toLowerCase());
        if (!relevantFile) {
    ...
    parseTablesFromMarkdown(markdown) {
            ...
            if (typeof obj === 'object' && obj !== null) {
                for (const [key, value] of Object.entries(obj)) {
                    ...
                    if (key.toLowerCase().includes(searchTerm.toLowerCase()) ||
                        (typedValue?.name && typedValue.name.toLowerCase().includes(searchTerm.toLowerCase()))) {
                        const result = { path: [...path, key], data: typedValue };
    ...
    async generateContent(type, name, parameters = {}) {
        try {
            // Load template
            ...
            return {
                content: [{
                        type: 'text',
                        text: `Error generating ${type} for ${name}: ${error.message}`,
                    }],
            };
        }
    }
    async getEntityData(name, type) {
        ...
        for (const file of markdownFiles) {
            ...
            for (const [tableName, tableData] of Object.entries(tables)) {
                if (Array.isArray(tableData)) {
                    const found = tableData.find((row) => Object.values(row).some((value) => typeof value === 'string' && value.toLowerCase().includes(name.toLowerCase())));
                    if (found) {
    ...
    async generateBasicTemplate(type) {
        const templates = {
...
*Generated from Aetheria class data*`,
    ...
    async extractData(source, dataType = 'all', format = 'json') {
        try {
            const markdownFiles = await this.findMarkdownFiles();
            const relevantFile = markdownFiles.find(f => path.basename(f, '.md').toLowerCase() === source.toLowerCase());
            if (!relevantFile) {
            ...
            return {
                content: [{
                        type: 'text',
                        text: `Error extracting data from ${source}: ${error.message}`,
                    }],
            };
        }
    }
